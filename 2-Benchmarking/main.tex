...
The use of a vehicle for landmine detection allows for much larger areas to be scanned in a shorter period of time. Vehicles can also carry greater loads than any human operator could, which means that more sensing equipment can be carried; this in turn increases sensing capability, and reduces the likelihood of false positives. If the vehicle can operate remotely or autonomously, the risk to human operators can be eliminated entirely. For these reasons, remotely controlled or autonomous platforms have become a popular tool for landmine detection. This section looks a number of platforms that are in operation or in development. 

\subsection{Military Mine Clearance}
\textbf{
...
% In military scenarios, the key priority is to clear a path as quickly as possible for a convoy \parencite{portugal2014}. In many cases, landmine detection is coupled with landmine clearance. 

% \textcolor{green}{there is meant to be a figure here but its breaking everything so ive removed it temporarily}
% \begin{figure}[ht]
% \centerline{
% \begin{tabular}{cc}
% \subfloat[PANAMA Snatch \parencite{think2012}]{\includegraphics[height=0.25\textwidth]{2-Benchmarking/panama.jpg}} 
%   & \subfloat[M1 Panther II \parencite{military2016}]{\includegraphics[height=0.25\textwidth]{2-Benchmarking/m1panther.jpg}}\\
%   \multicolumn{2}{c}{\subfloat[FORESIGHT RDV \parencite{canada2004}]{\includegraphics[height=0.25\textwidth]{2-Benchmarking/foresight.jpg}}}
% \end{tabular}}
% \caption{Military vehicles for landmine detection}
% \figlabel{military}
% \end{figure}
 
% \Figref{military} shows three types of remotely controlled or autonomous landmine detection vehicles that have been used in military operations. 
...
% \paragraph{M1 Panther II} Based on the M1 Abrams battle tank, the M1 Panther II consists of a vehicle mounted demining device (VMDD) attached to the body of the tank \parencite{military2016}. The VMDD is often a mine roller, a set of heavy rollers that cause mines to detonate, or a mine plough, which pushes mines out of the path of the vehicle. Being an armoured vehicle, the the Panther is capable of withstanding damage from smaller munitions. Asides from manual operation, the vehicle can be controlled remotely from up to 800 m away \parencite{military2016}.
% \nomenclature[A]{VMDD}{Vehicle Mounted Demining Device}%
...
% \centerline{
% \begin{tabular}{cc}
% \subfloat[DOK-ING MV-10 \parencite{doking2013}]{\includegraphics[height=0.25\textwidth]{2-Benchmarking/mv10.jpg}} 
%    & \subfloat[NIITEK Minestalker \parencite{niitek2015}]{\includegraphics[height=0.25\textwidth]{2-Benchmarking/minestalker.jpg}}\\
%    \multicolumn{2}{c}{\subfloat[Digger D-250 \parencite{digger2016}]{\includegraphics[height=0.25\textwidth]{2-Benchmarking/d-250.jpg}}}
% \end{tabular}}
% \caption{Commercially available vehicles for landmine detection}
% \figlabel{commercial}
% \end{figure}

% \subsection{Humanitarian Demining}
% Unlike with military mine clearance, humanitarian demining requires greater levels of accuracy and effort. A clearance rate of 80\% is generally accepted in combat, a figure which only needs to apply to pathways where troops or convoys will travel; for humanitarian situations, the area required to be cleared is much larger, and such a low clearance rate cannot be accepted \parencite{habib2008}. Vehicles available for humanitarian landmine detection can be sorted into two categories, commercially available (\Figref{commercial}) and research vehicles (\Figref{research}). The former are often smaller scale versions of similar devices used in the military, while the the latter are in development by research organisations, and are yet to be deployed.

% \subsubsection{Commercially Available Vehicles}
% \paragraph{DOK-ING MV-10} The MV-10 is essentially a medium sized, remote control VMDD. Designed only for mine clearance, the vehicle is equipped with a mine flail, a rotating tool that detonates mines in impact, and a mine tiller, which can uncover buried mines \parencite{doking2013}. An operator can operate the MV-10 from a distance of 2 km, and its small size makes it easier to transport than a military system. 
...
% \paragraph{Digger D-250} The D-250 is quite similar to the MV-10. Both are treaded vehicles of a similar size, and both use a flail and tiller combination for landmine clearance. The operating range of the D-250 is smaller at 500 metres \parencite{digger2016}.

% \subsubsection{Research Vehicles}
...
%The following subsections will discuss the specific experimental set-ups that have been implemented in order to meet similar objectives as specified in \textcolor{red}{reference to primary objective}.  
